...
export class ReportService {
  ...
  async BalanceQuery(user, value: any = null, type = null) {
    ...
    if ((value == null || value == []) && type == null) {
     ...
     `;
      ...
      const balance_query = `case when primary_accounts.name='assets' OR primary_accounts.name='expense'
    ...
    `;
     ...
     and transactions.date >= '${today_date}' and transactions.date <= '${add_one_day}'
      `;
        ...
        const opening_balance = `case when primary_accounts.name='assets' OR primary_accounts.name='expense'
      then ( (${_debits})-(${_credits}))
      else (( ${_credits})-(${_debits})) end`;

        const _balance = `case when primary_accounts.name='assets' OR primary_accounts.name='expense'
      then ( (${debits})-(${credits}))
  ...
  async BalanceSheet(user, query) {
    let main_query;

    if (query !== undefined) {
      const filter_data: any = Buffer.from(query, 'base64').toString();
    ...
    const newResult = ['asset', 'equity', 'liability']
      .map(item => {
        ...
        if (filtered.length > 0) {
          let { balance } = filtered.reduce((a, b) => {
  ...
  async TrailBalance(user, query = null) {
    try {
      ...
      let totalCredits = (newResult?.length &&
        newResult?.reduce((a, b) => {
          return { credit: a.credit + b.credit };
        })) || { credit: 0 };

      let totalDebits = (newResult?.length &&
        newResult?.reduce((a, b) => {
