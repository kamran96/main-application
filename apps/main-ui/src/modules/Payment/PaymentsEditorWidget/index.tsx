...
export const PaymentsEditorWidget: FC = () => {
  ...
  useEffect(() => {
    if (history && history.location && history.location.search) {
      let type = history.location.search.split('=')[1];
      if (type === 'received') {
        setFormData({ ...formData, paymentMode: IPaymentsMode.RECEIVABLES });
        form.setFieldsValue({ paymentMode: IPaymentsMode.RECEIVABLES });
      } else {
        setFormData({ ...formData, paymentMode: IPaymentsMode.PAYABLES });
        form.setFieldsValue({ paymentMode: IPaymentsMode.PAYABLES });
      }
  ...
  const onFinish = async (values) => {
    ...
    if (!values.runningPayment) {
      payload = {
        ...payload,
        payment: { ...payload.payment, invoice_ids: [...paid_invyces] },
      };
    }
