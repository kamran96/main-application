  ...
  const columns: ColumnsType<any> = [
    {
      ...
      render: (data, row)=>{
        return (
          <Button onClick={()=> setTaxForm({visibility: true, id: row?.id})} type="link">{data? data : ''}</Button>
        )
      }
    },
    {
  ...
  return (
    <WrapperTaxList>
      <div className="flex justifySpaceBetween">
        ...
        <Button onClick={() => setTaxForm({visibility: true, id: null})} type="primary" size="middle">
          Add Tax
        </Button>
      </div>
      <CommonTable
        ...
        enableRowSelection
      />
      <TaxEditorWidget visibility={visibility} id={id} setVisibility={(visibility)=>setTaxForm({visibility, id: null})} />
    </WrapperTaxList>
  );
};
