...
export const ProcessedQuotations: FC<IProps> = ({ columns }) => {
  ...
  const { mutate: mutateDeleteOrders, isLoading: deletingInvoiceDraft } =
      ...
      <div className="flex flexCenter">
        ...
        <SmartFilter
          onFilter={(encode) => {
            ...
            const route = `/app${ISupportedRoutes.QUOTE}?tabIndex=processed&sortid=${sortid}&page=1&page_size=${pageSize}&sortid=${sortid}&query=${encode}`;
            history.push(route);
          }}
      ...
      <CommonTable
        ...
        onChange={(pagination, filters, sorter: any, extra) => {
          if (sorter.order === undefined) {
            ...
            const route = `/app${ISupportedRoutes.QUOTE}?tabIndex=processed&sortid=${sortid}&page=${pagination.current}&page_size=${pagination.pageSize}&query=${query}`;
            history.push(route);
          } else {
            setAllInvoicesConfig({
              ...allInvoicesConfig,
