...
export const AwaitingAprovalBillsList: FC<IProps> = ({ columns }) => {
  ...
  const { isLoading, resolvedData, isFetching } = usePaginatedQuery(
    [
      `awaiting_aproval?page=${page}&query=${query}&sort=${sortid}&page_size=${pageSize}`,
      ORDER_TYPE.BILL,
      INVOICETYPE.Awaiting_Approval,
      page,
      pageSize,
      sortid,
    ],
    ...
    <CommonTable
          ...
          renderFilter={
            <SmartFilter
              onFilter={(encode) => {
                ...
                let route = `/app${ISupportedRoutes.BILLS}?tabIndex=awating_aproval&sortid=null&page=1&page_size=${pageSize}&sortid=${sortid}&query=${encode}`;
                history.push(route);
              }}
      ...
      onChange={(pagination, filters, sorter: any, extra) => {
        if (sorter.order === undefined) {
          ...
          let route = `/app${ISupportedRoutes.BILLS}?tabIndex=awating_aproval&sortid=null&page=${pagination.current}&page_size=${pagination.pageSize}&query=${query}`;
          history.push(route);
        } else {
          setAllInvoicesConfig({
            ...allInvoicesConfig,
          ...
          }?tabIndex=awating_aproval&sortid=null&page=${
            pagination.current
