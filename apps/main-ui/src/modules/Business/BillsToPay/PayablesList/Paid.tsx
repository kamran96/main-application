...
export const PaidBillsList: FC<IProps> = ({ columns }) => {
  ...
  const { isLoading, resolvedData, isFetching } = usePaginatedQuery(
    [
      `paid_bills?page=${page}&query=${query}&sort=${sortid}&page_size=${pageSize}`,
      ORDER_TYPE.BILL,
      INVOICETYPE.PAID,
      page,
      pageSize,
    ...
    <CommonTable
          ...
          renderFilter={
            <SmartFilter
              onFilter={(encode) => {
                ...
                let route = `/app${ISupportedRoutes.BILLS}?tabIndex=paid&sortid=${sortid}&page=1&page_size=${pageSize}&sortid=${sortid}&query=${encode}`;
                history.push(route);
              }}
      ...
      onChange={(pagination, filters, sorter: any, extra) => {
        if (sorter.order === undefined) {
          ...
          let route = `/app${ISupportedRoutes.BILLS}?tabIndex=paid&sortid=${sortid}&page=${pagination.current}&page_size=${pagination.pageSize}&query=${query}`;
          history.push(route);
        } else {
          setAllInvoicesConfig({
            ...allInvoicesConfig,
          ...
          }?tabIndex=paid&sortid=null&page=${
            pagination.current
