...
export const AwaitingPaymentBillsList: SFC<IProps> = ({ columns }) => {
  ...
  const { isLoading, resolvedData, isFetching } = usePaginatedQuery(
    [
      `awaiting_payment_bills?page=${page}&query=${query}&sort=${sortid}&page_size=${pageSize}`,
      ORDER_TYPE.BILL,
  ...
  }, [routeHistory, allInvoicesConfig, history]);
    ...
    <CommonTable
          ...
          renderFilter={
            <SmartFilter
              onFilter={(encode) => {
                ...
                let route = `/app${ISupportedRoutes.BILLS}?tabIndex=awating_payment&sortid=null&page=1&page_size=${pageSize}&sortid=${sortid}&query=${encode}`;
                history.push(route);
              }}
      ...
      onChange={(pagination, filters, sorter: any, extra) => {
        if (sorter.order === undefined) {
          ...
          let route = `/app${ISupportedRoutes.BILLS}?tabIndex=awating_payment&sortid=null&page=${pagination.current}&page_size=${pagination.pageSize}&query=${query}`;
          history.push(route);
        } else {
          setAllInvoicesConfig({
            ...allInvoicesConfig,
