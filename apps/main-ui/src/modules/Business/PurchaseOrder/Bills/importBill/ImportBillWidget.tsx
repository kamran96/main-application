...
const ImportBillWidget: FC = () => {
  const { bills, setBills } = useGlobalContext();
  const { visibility } = bills;
          ...
          {state && (
            <div className="render-content">
              ...
              <InvoiceImportManager
                headers={`Order Number,Reference,Return,Supplier,Date Raised,Delivery Date,Paid Amount,Due Amount,Total`.split(
                  ','
                )}
                onLoad={(payload, file) => {
                  setFileData(file);
...
export default ImportBillWidget;
