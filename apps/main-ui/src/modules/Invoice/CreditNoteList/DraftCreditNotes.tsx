    ...
    const renderTopbarRight = () => {
      return (
        <div className="flex alignCenter">
          ...
          <SmartFilter
            onFilter={(encode) => {
              let route = `/app${ISupportedRoutes?.CREDIT_NOTES}?tabIndex=draft&page=1&page_size=20&query=${encode}`;
              history.push(route);
      ...
      <CreditNoteWrapper>
        <CommonTable
          ...
          onChange={(pagination, filters, sorter: any, extra) => {
            ...
            let route = `/app${ISupportedRoutes.CREDIT_NOTES}?tabIndex=draft&page=${pagination.current}&page_size=${pagination.pageSize}&query=${query}`;
            history.push(route);
          }}
          pagination={{
            pageSize: pageSize,
